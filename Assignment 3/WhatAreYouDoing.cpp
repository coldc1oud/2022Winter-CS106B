 ...
 */
void  meiju(Vector<bool>& pos,Vector<bool>& ans,int num,int endnum,Set<Vector<bool>>& total){
    if(num==endnum || num == pos.size()) {
        total+=ans;
        return;
    }
    if(pos[num] == 0 ){
        ans[num] = 0;
        meiju(pos, ans, num+1, endnum,total);
    }
    else {
        ans[num] = 1;
        meiju(pos, ans, num+1, endnum,total);
...
Set<string> allEmphasesOf(const string& sentence) {
    Vector<string> str = tokenize(sentence);
    Vector<bool> pos(str.size(), 0);
    Set<Vector<bool>> total;
    int poss = str.size();
    int num=0;
    for(string s : str ){
        if(isalpha(s[0]) != 0 ) {
    ...
    Vector<bool> ans(str.size(), 0);
    meiju(pos, ans, 0, poss,total);
    for(Vector<bool> p : total) {
        string nowstr;
        for(int i = 0 ; i < str.size() ; i++){
            if(p[i] == 0) {
                string tempstr = toLowerCase(str[i]);
                nowstr += tempstr;
            }
            else {
                string tempstr = toUpperCase(str[i]);
                nowstr += tempstr;
            }
        }
        allEmphases.add (nowstr);
    }
    return allEmphases;
}
