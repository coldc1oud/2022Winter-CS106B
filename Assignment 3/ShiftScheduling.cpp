...
void solve2(Vector<Shift>& shifts , int deep,
            int nowHours, int maxHours, int sum, int& max,
             Set<Shift>& res, Set<Shift> now,
            Set<Shift> notshift, Map<Shift,Set<Shift>>& nots){
    if(deep == shifts.size() ) {
        if (sum > max ){
            max = sum;
            res = now;
    ...
    if(nowHours + lengthOf(shifts[deep]) <= maxHours &&
                  notshift.contains(shifts[deep]) == 0) {
        Set<Shift> newshift = notshift + nots[shifts[deep]];
        now.add(shifts[deep]);
        solve2(shifts, deep+1, nowHours + lengthOf(shifts[deep]), maxHours,
              sum + valueOf(shifts[deep]), max, res, now, newshift, nots);
        now.remove(shifts[deep]);
    }
    solve2(shifts, deep+1, nowHours, maxHours,
           sum, max, res, now, notshift, nots);
}
/*void solve(Vector<Vector<Shift>>& dp,
           int sum,
           int& max,int deep,
           Set<Shift>& res,Set<Shift> now,
           int maxHours, int nowHours) {
    if(deep == dp.size() ) {
        if (sum > max ){
            max = sum;
            res = now;
    ...
    for(Shift shift : dp[deep]) {
        if(nowHours + lengthOf(shift) > maxHours){
           continue;
        }
        else {
            now.add(shift);
            solve(dp, sum+valueOf(shift), max, deep+1, res, now, maxHours, nowHours+lengthOf(shift));
            now.remove(shift);
        }
    }
    solve(dp, sum, max, deep+1, res, now, maxHours, nowHours);
    return;
} */
Set<Shift> highestValueScheduleFor(const Set<Shift>& shifts, int maxHours) {
    ...
    Map<Shift, Set<Shift>> pos;
    int length = total.size();
    for(int i = 0; i < length; i++) {
        for(int j = i+1; j < length; j++){
            if(overlapsWith(total[i],total[j]) != 0 ){
                pos[total[i]].add(total[j]);
            }
        }
    }
   /* Vector<Vector<Shift>> dp;
    for(Shift s : shifts) {
        if(dp.isEmpty() != 0) {
            Vector<Shift> p={s};
            dp.add(p);
        ...
        bool flag = 0;
        for(Vector<Shift>& a : dp) {
            if(overlapsWith(a[0],s) != 0) {
                a.add(s);
                flag = 1;
                break;
            }
    ...
    Set<Shift> now;
    int max = 0;
    solve(dp, 0, max, 0, res, now, maxHours, 0);
    return res; */
    Set<Shift> res;
    Set<Shift> now;
    Set<Shift> notshift;
    int max = 0;
    solve2(total, 0, 0, maxHours, 0, max, res, now, notshift, pos);
    return res;
}
