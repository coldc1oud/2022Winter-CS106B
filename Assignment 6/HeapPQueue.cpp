...
void HeapPQueue::enqueue(const DataPoint& data) {
    if (logicalSize == allocatedSize - 1) {
        DataPoint* newelems = new DataPoint[2 * allocatedSize];
    ...
    *(elems + logicalSize )= data;
    int parentnum = logicalSize / 2;
    int nownum = logicalSize;
    while((*(elems + parentnum)).weight > data.weight && parentnum >= 1) {
        swap(elems[parentnum], elems[nownum] );
            nownum = parentnum;
          parentnum = nownum / 2;
    }
}
...
void HeapPQueue::swapdequeue(int parentnum, int nownum) {
    if(parentnum == 1) {
        swap(elems[parentnum],elems[nownum]);
        return;
    }
    if(parentnum%2 == 0) {
        if(elems[parentnum].weight <= elems[parentnum+1].weight) {
            swapdequeue(parentnum/2, parentnum);
            swap(elems[parentnum],elems[nownum]);
        }
        else{
            swapdequeue((parentnum+1)/2, parentnum+1);
            swap(elems[parentnum+1],elems[nownum]);
        }
    }
    else {
        if(elems[parentnum].weight <= elems[parentnum-1].weight) {
...
DataPoint HeapPQueue::dequeue() {
   ...
   /* swapdequeue(logicalSize/2, logicalSize);
    logicalSize --;
    return elems[logicalSize+1]; */
    ...
    int leftnum = 2;
    int rightnum = 3;
    while(leftnum <= logicalSize) {
        int weight = elems[nownum].weight;
        int leftweight = elems[leftnum].weight;
        int rightweight = 0;
        if(rightnum <= logicalSize) {
            rightweight = elems[rightnum].weight;
        }
        if(leftweight <= rightweight || rightnum > logicalSize) {
            if(leftweight <= weight) {
                swap(elems[nownum], elems[leftnum]);
                nownum = leftnum;
            }
            else {
                break;
            ...
            if(rightweight <= weight) {
                swap(elems[nownum],elems[rightnum]);
                nownum = rightnum;
            }
            else {
                break;
