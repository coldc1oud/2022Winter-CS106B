...
#include "Combine.h"
using namespace std;
Vector<DataPoint> msort (const Vector<Vector<DataPoint>>& sequences, int l,int r ) {
    if(l >= r) return sequences[l];
    int mid = (l+r) / 2;
    Vector<DataPoint> left;
    Vector<DataPoint> right;
    left = msort(sequences, l, mid);
    right = msort(sequences, mid+1, r);
    Vector<DataPoint> res;
    int posl = 0;
    int posr = 0;
    while(posl < left.size() && posr < right.size()) {
        DataPoint leftnum = left[posl];
        DataPoint rightnum = right[posr];
        if(leftnum.weight <= rightnum.weight) {
            res.add(leftnum);
            posl ++;
        }
        else  {
...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    Vector<DataPoint> combine;
    if(sequences.size() == 0) {
        return combine;
    }
    combine = msort(sequences, 0, sequences.size()-1);
    return combine;
}
