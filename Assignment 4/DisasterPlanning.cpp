...
bool res=0;
int maxnum;
void solve(const Vector<string>& city, const Map<string, Set<string>>& roadNetwork, int deep, int num,
           Set<string>& supplyLocation, Set<string> now,Set<string> already){
    if(already.size() == city.size()) {
        res = 1;
        supplyLocation = now;
        return;
    }
    if(deep == city.size() || num == maxnum )  {
        return ;
    }
    string nowcity = city[deep];
    if(already.contains(nowcity) && city.size() >= 20) {
          solve(city, roadNetwork, deep+1, num, supplyLocation, now, already);
          return;
    }
    now.add(nowcity);
    Set<string> newalready;
    newalready = already;
    newalready.add(nowcity);
    newalready += roadNetwork[nowcity];
    now.add(nowcity);
    solve(city, roadNetwork, deep+1, num+1, supplyLocation, now, newalready);
    if(res == 1) {
        return;
    }
    now.remove(nowcity);
...
bool canBeMadeDisasterReady(const Map<string, Set<string>>& roadNetwork,
    ...
    Set<string> now;
    Set<string> already;
    solve(city, roadNetwork, 0, 0, supplyLocations, now, already);
    return res;
