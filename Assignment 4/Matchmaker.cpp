...
using namespace std;
void matchingsolve(Vector<string> people, const Map<string, Set<string>>& possibleLinks, bool& ans, int deep,
                   Set<Pair>& matching, Set<Pair> now, int matchingnum, Set<string> have) {
    if(deep == people.size()) {
        if(matchingnum == people.size()/2) {
            ans = 1;
            matching = now;
            return;
        }
    ...
    if(have.contains(people[deep]) != 0) {
        matchingsolve(people, possibleLinks, ans, deep+1, matching, now, matchingnum, have);
    }
    else {
        for(string linkTo: possibleLinks[people[deep]]) {
            if(have.contains(linkTo) == 0) {
                now.add({people[deep], linkTo});
                have.add(people[deep]);
                have.add(linkTo);
                matchingsolve(people, possibleLinks, ans, deep+1, matching, now, matchingnum+1, have);
                if(ans == 1) {
                    return;
                }
                have.remove(linkTo);
                have.remove(people[deep]);
                now.remove({people[deep],linkTo});
            }
        }
    }
    return;
}
void bestmatchingsolve(Vector<string>& people, const Map<string, Set<string>>& possibleLinks, int sum, int& max, int deep,
                   Set<Pair>& matching, Set<Pair> now, Set<string> have,
                      const Map<string, Map<string, int>>& data) {
    if(deep == people.size()) {
        if (sum > max) {
            max = sum;
            matching = now;
    ...
    if(have.contains(people[deep]) != 0) {
        bestmatchingsolve(people, possibleLinks, sum, max, deep+1, matching, now,  have, data);
    }
    else {
        for(string linkTo: possibleLinks[people[deep]]) {
            if(have.contains(linkTo) == 0) {
               /* if(already.contains({linkTo, people[deep]})) {
                    continue;
                } */
                now.add({people[deep], linkTo});
                have.add(people[deep]);
                have.add(linkTo);
                bestmatchingsolve(people, possibleLinks, sum + data[people[deep]][linkTo], max, deep+1, matching, now, have, data);
                have.remove(linkTo);
                have.remove(people[deep]);
                now.remove({people[deep],linkTo});
            }
...
bool hasPerfectMatching(const Map<string, Set<string>>& possibleLinks, Set<Pair>& matching) {
    ...
    bool ans = 0;
    matchingsolve(people, possibleLinks, ans, 0, matching, now, 0, have);
    return ans;
}
...
Set<Pair> maximumWeightMatching(const Map<string, Map<string, int>>& possibleLinks) {
    ...
    for(string s : possibleLinks) {
        for(string str : possibleLinks[s]){
            if(links[str].contains(s)) {
                continue;
            }
            links[s].add(str);
        }
    ...
    Set<Pair> matching;
    int max = 0;
     bestmatchingsolve(people, links, 0, max, 0, matching, now,  have, possibleLinks);
    return matching;
}
