...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    ...
    if(kGramLength <= 0) {
        error("kGralength is not true!");
    }
    else if(kGramLength <= length) {
        for(int i = 0; i <= length - kGramLength ; i++){
            string newstr = str;
            newstr = newstr.substr(i, kGramLength);
            kgrams[newstr] += 1;
        }
    }
    return kgrams;
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    ...
    if(numToKeep<0){
        ...
        int numsource = source.size();
        for(string str : source) {
            p.enqueue(str, source[str]);
        }
        int sub = numsource - numToKeep;
        if(sub < 0 ) {
            sub=0;
        }
        for(int i = 0;i < sub ; i++) {
    ...
    while(!p.isEmpty()) {
        string str = p.dequeue();
        topKGramsIn[str] = source[str];
    }
    return topKGramsIn;
}
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    for(Corpus cor : corpora) {
        double nownum = cosineSimilarityOf(textProfile ,cor.profile) ;
        if(nownum>maxnum){
            name = cor.name;
            maxnum=nownum;
        }
