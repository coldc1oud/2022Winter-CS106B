...
bool LinearProbingHashTable::insert(const string& elem) {
    if (elenum == vecnum || this->contains(elem) == 1) {
        return 0;
    }
    int hashcode = nowhash(elem);
    for(int i = hashcode % vecnum ,  j = 0; j < vecnum ;j++ ){
        if (elems[i].type == SlotType::EMPTY || elems[i].type == SlotType::TOMBSTONE) {
            elems[i].value = elem;
