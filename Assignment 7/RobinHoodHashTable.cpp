...
bool RobinHoodHashTable::isEmpty() const {
    return elenum?false:true;
}

bool RobinHoodHashTable::insert(const string& elem) {
    if(elenum == vecnum || this->contains(elem)) {
        return false;
    }
    int hashcode = now(elem);
    Slot pos = {elem,0};
   for(int i = hashcode % vecnum, cnt = 0;cnt < vecnum; cnt++,i = (i+1)%vecnum ) {
        if(elems[i].distance == EMPTY_SLOT) {
            elems[i] = pos;
            elenum ++;
            return true;
        }
        if(elems[i].distance >= pos.distance) {
            pos.distance ++;
            continue;
        }
        else {
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    for(int i = hashcode % vecnum, cnt = 0;cnt < vecnum; cnt++,i = (i+1)%vecnum ) {
         ...
         if(elems[i].distance < cnt || elems[i].distance == EMPTY_SLOT) {
            return false;
        }
    }
    return false;
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    for(int i = hashcode % vecnum, cnt = 0;cnt < vecnum; cnt++,i = (i+1)%vecnum ) {
        ...
        if(flag == 1 && elems[(i+1)%vecnum].distance == 0) {
            elems[i].distance = EMPTY_SLOT;
            elenum --;
            return true;
        }
        if(flag == 1 && (elems[(i+1)%vecnum].distance ==EMPTY_SLOT)) {
            elems[i].distance = EMPTY_SLOT;
            elenum --;
            return true;
        }
        if(flag == 1 && cnt == vecnum - 1 ) {
            //swap(elems[i],elems[(i+1)%vecnum]);
            elems[(i+1)%vecnum].distance = EMPTY_SLOT;
            elenum --;
            return true;
        }
        if(flag == 1) {
