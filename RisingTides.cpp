...
Grid<bool> bfs(const Grid<double>& terrain,
               int x,int y,double height,
                Grid<bool>& regions){
    Queue<node> p;
    Grid<bool> vis(terrain.numRows(),terrain.numCols(),false);
    p.enqueue({x,y});
   // Grid<bool> regions(terrain.numCols(),terrain.numRows(),false);
    ...
    while(!p.isEmpty()){
        ...
        int nowy = now.y;
        //vis[nowx][nowy] = true;
        if(terrain[nowx][nowy] <= height) {
            regions[nowx][nowy] = true;
            for(int i = 0;i < 4; i++) {
                int tx = nowx + xx[i];
                int ty = nowy + yy[i];
                if(tx < 0 ||ty < 0 ||tx >= terrain.numRows() || ty >= terrain.numCols()) continue;
...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
                            ...
                            double height) {
     Grid<bool> regions(terrain.numRows(),terrain.numCols(),false);
    for(GridLocation loc : sources) {
        int x = loc.row;
        int y = loc.col;
        regions = bfs(terrain,x,y,height, regions);
    }
    return regions;
}
